...
if __name__ == '__main__':
    ...
    while(1):
        ...
        while Q:
            ...
            for adj_node in Vdict[node_index][0]:
                ...
                if v[0] > u[0] + adj_node[1]:   # if d(src, v) > d(src, u) + w
                    ...
                    del Vdict[adj_node_index][1][:]
                    Vdict[adj_node_index][1].append((node_index, adj_node[1]))
                                                # Construct parent tree
                elif v[0] == u[0] + adj_node[1]:
                    Vdict[adj_node_index][1].append((node_index, adj_node[1]))
        ...
        while Q:
            ...
            for adj_node in Vdict[node_index][1]:
                adj_node_index = adj_node[0]
                v = Qdict[adj_node_index]       # Adjacent node's dist estimate
                if v[0] > u[0] + adj_node[1]:   # if d(src, v) > d(src, u) + w
                    v_new = v[:]                # Create a copy of the Q elem
        ...
        def find_split_point(node_index, split_point):
            for adj_node in Vdict[node_index][1]:
                adj_node_index = adj_node[0]
                v = Qdict[adj_node_index]
                if v[0] < split_point[0]:
                    split_point[0] = v[0]
                    split_point[1] = v[1]
                find_split_point(adj_node_index, split_point)
        
        node_index = n
        split_point = [float('inf'), -1]
        find_split_point(node_index, split_point)
        
        # This is the distance to the split point from Charly's house
        split_point_dist = split_point[0]
        print (c_min_dist - split_point_dist), 
        print split_point_dist, (n_min_dist + split_point_dist - c_min_dist)
